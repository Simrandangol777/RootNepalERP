...
const AddEditProductModal = ({ product, onClose, onSave }) => {
  ...
  return (
    <div className="product-modal fixed inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center p-4 z-50">
      <div className="product-modal-shell bg-gradient-to-br from-slate-900 via-purple-900 to-slate-900 rounded-2xl w-full max-w-4xl max-h-[90vh] overflow-hidden border border-white/20 shadow-2xl">
        <div className="product-modal-header bg-white/10 backdrop-blur-xl border-b border-white/20 px-6 py-4 flex items-center justify-between">
          <h2 className="text-2xl font-bold text-white">
            {product ? "Edit Product" : "Add New Product"}
          </h2>
          <button
            onClick={onClose}
            className="product-modal-close p-2 text-white/70 hover:text-white hover:bg-white/10 rounded-lg transition-all"
          >
        ...
        <form onSubmit={handleSubmit} noValidate>
          <div className="product-modal-body overflow-y-auto p-6 space-y-6" style={{ maxHeight: "calc(90vh - 140px)" }}>
            {formError && (
              <div className="product-modal-message rounded-xl border border-red-500/30 bg-red-500/10 px-4 py-3 text-sm text-red-200">
                {formError}
              </div>
            )}
            <div className="product-modal-panel bg-white/5 border border-white/10 rounded-2xl p-6">
              <label className="text-sm font-semibold text-white mb-3 block">Product Image</label>
              <div className="flex items-start gap-6">
                <div
                  onClick={() => fileInputRef.current?.click()}
                  className="product-modal-upload w-48 h-48 rounded-xl bg-white/5 border-2 border-dashed border-white/20 flex flex-col items-center justify-center cursor-pointer hover:bg-white/10 hover:border-white/30 transition-all overflow-hidden"
                >
                  {imagePreview ? (
                ...
                <div className="flex-1 space-y-2">
                  ...
                  {imagePreview && (
                    <button
                      ...
                      className="product-modal-remove text-red-400 hover:text-red-300 text-sm flex items-center gap-1 mt-2"
                    >
            ...
            <div className="product-modal-panel bg-white/5 border border-white/10 rounded-2xl p-6 space-y-4">
          ...
          <div className="product-modal-footer bg-white/10 backdrop-blur-xl border-t border-white/20 px-6 py-4 flex items-center justify-end gap-3">
            <button
              type="button"
              onClick={onClose}
              className="product-modal-secondary px-6 py-3 bg-white/5 hover:bg-white/10 text-white rounded-xl transition-all"
            >
              Cancel
            </button>
            <button type="submit" disabled={isLoading} className="product-modal-primary group relative px-6 py-3">
